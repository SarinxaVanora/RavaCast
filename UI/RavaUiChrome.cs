...
internal static class RavaUiChrome
{
    ...
    public static void DrawHero(string title, string subtitle, string? rightText = null, Vector4? rightColor = null, Action? content = null, float contentHeight = 0f)
    {
        ...
        var rightReserve = !string.IsNullOrWhiteSpace(rightText) ? ImGui.CalcTextSize(rightText).X + (46f * scale) : 0f;
        var textWidth = MathF.Max(90f * scale, width - pad.X * 2f - rightReserve);
        ...
        var height = MathF.Max((hasSubtitle ? 58f : 44f) * scale, pad.Y * 2f + textHeight + extraHeight);
    ...
    public static void DrawSectionTitle(string title, string? subtitle = null)
    {
        ...
        var textWidth = MathF.Max(90f * scale, width - pad.X * 2f);
